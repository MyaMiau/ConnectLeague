  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center py-10 px-4">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        />
        <h2 className="text-2xl font-semibold mt-4 mb-2">{org.name}</h2>
        <p className="text-zinc-400 mb-2">{org.email}</p>
        {org.bio ? (
          <p className="text-zinc-300 mb-2">{org.bio}</p>
        ) : (
          <p className="italic text-zinc-600 mb-2">Nenhuma descrição ainda.</p>
        )}
        {/* Botão de edição se for a própria org logada */}
        {org.isCurrentUser && (
          <Button onClick={() => router.push(`/organization/edit?id=${org.id}`)}>Editar Perfil</Button>
        )}
      </div>
          ...
          <div className="space-y-4">
            {vagas.map(vaga => (
              <div key={vaga.id} className="bg-zinc-900 rounded-xl p-5 shadow">
                ...
                <span className="text-xs text-zinc-400">Publicada em {vaga.created_at ? new Date(vaga.created_at).toLocaleDateString() : "?"}</span>
                <div className="mt-2">
                  <Button size="sm" onClick={() => router.push(`/vagas/${vaga.id}`)}>Ver vaga</Button>
                </div>
              </div>
            ))}
          </div>
